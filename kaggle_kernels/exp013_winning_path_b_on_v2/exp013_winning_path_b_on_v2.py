...
# Same logic as exp011, but applied to exp009 v2 base (= LB 9.738 = current best).
# Dataset: ky7240/rogii-winning-path-b-features
#   contains b-cluster-xy / per-well-high-order / formation-uniqueness.
...
_WINPATH_B_LOOKUP: dict = {}  # populated lazily in build_well_features
...
def build_edge_q_folds(
    ...
    CV-strategy override (= task kaggle-rogii-cv-strategies-2026-05-11):
        ...
        columns. When set, the typewell-hash fold construction is ignored
        and the override mapping is applied so the kernel can be re-run
        under each of the 5 CV strategies (baseline / C1 / C2 / C3 / C4).
    """
    import os as _os
...
# ────────────────────────────────────────────────────────────────────────────
# Winning Path B (= D1 + D4 + D9) lookup helpers (= exp013 inject、 from exp011)
# ────────────────────────────────────────────────────────────────────────────
...
def winpath_b_load(dataset_root: str = WINPATH_B_DATASET_ROOT,
    ...
    if h_path.exists():
        ...
        if "b_jump_max_abs" in hdf.columns:
            hdf = hdf[["well_id", "b_jump_max_abs"]].drop_duplicates(subset=["well_id"])
...
def build_well_features(
    ...
    # ── Winning Path B (= D1 + D4 + D9) inject (= exp013 新規) ──────────────
    if WINPATH_B_ENABLE:
