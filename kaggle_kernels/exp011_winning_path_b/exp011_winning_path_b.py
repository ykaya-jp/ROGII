...
# ─── Winning Path B knobs (= deepest EDA D1+D4+D9 features inject) ─────────
# Dataset: ky7240/rogii-winning-path-b-features
#   contains 3 parquets from outputs/eda/deepest_eda/:
#     b-cluster-xy.parquet         (= D1 source: 67 b_well clusters)
#     per-well-high-order.parquet  (= D4 source: b_jump_max_abs)
#     formation-uniqueness.parquet (= D9 source: 6 formation top medians)
# Loaded once at kernel start, looked up per-well in build_well_features.
WINPATH_B_ENABLE       = True
WINPATH_B_DATASET_ROOT = "/kaggle/input/rogii-winning-path-b-features"
WINPATH_B_ROUND_TO     = 3  # b_ANCC_med rounding for 67-cluster snapshot
_WINPATH_B_LOOKUP: dict = {}  # populated at startup (= well_id → {col: value})
...
# ────────────────────────────────────────────────────────────────────────────
# Winning Path B (= D1 + D4 + D9) lookup helpers (= exp011 inject)
# ────────────────────────────────────────────────────────────────────────────

WINPATH_B_FORMATION_COLS = (
    "top_ANCC_med", "top_ASTNU_med", "top_ASTNL_med",
    "top_EGFDU_med", "top_EGFDL_med", "top_BUDA_med",
)
WINPATH_B_OUT_COLS = (
    "wpB_cluster_id",         # D1 (= categorical int)
    "wpB_b_jump_max_abs",     # D4 (= scalar float)
    "wpB_top_ANCC_med",       # D9.1
    "wpB_top_ASTNU_med",      # D9.2
    "wpB_top_ASTNL_med",      # D9.3
    "wpB_top_EGFDU_med",      # D9.4
    "wpB_top_EGFDL_med",      # D9.5
    "wpB_top_BUDA_med",       # D9.6
)


def winpath_b_load(dataset_root: str = WINPATH_B_DATASET_ROOT,
                   round_to: int = WINPATH_B_ROUND_TO) -> dict:
    """Build well_id → {col: value} lookup once.

    D1 (b_well_cluster_id):
        b-cluster-xy.parquet  →  b_ANCC_med (= TVT + Z - ANCC per-well median)
        round to ``round_to`` decimals (= 3 reproduces deepest EDA 67-cluster
        snapshot) then encode as integer 0..n_clusters-1.
    D4 (b_jump_max_abs):
        per-well-high-order.parquet  →  b_jump_max_abs (float).
    D9 (formation top medians, 6 cols):
        formation-uniqueness.parquet  →  top_{ANCC,ASTNU,ASTNL,EGFDU,EGFDL,BUDA}_med.
    """
    root = Path(dataset_root)
    out: dict = {}

    # D1
    b_path = root / "b-cluster-xy.parquet"
    if b_path.exists():
        bdf = (
            pd.read_parquet(b_path)[["well_id", "b_ANCC_med"]]
            .drop_duplicates(subset=["well_id"])
        )
        bdf["well_id"] = bdf["well_id"].astype(str)
        keys = bdf["b_ANCC_med"].round(round_to) if round_to > 0 else bdf["b_ANCC_med"]
        uniq = sorted(keys.dropna().unique().tolist())
        key_to_int = {k: i for i, k in enumerate(uniq)}
        bdf["cluster_id"] = keys.map(key_to_int).fillna(-1).astype(int)
        for wid, cid in zip(bdf["well_id"], bdf["cluster_id"]):
            out.setdefault(wid, {})["wpB_cluster_id"] = int(cid)

    # D4
    h_path = root / "per-well-high-order.parquet"
    if h_path.exists():
        hdf = pd.read_parquet(h_path)
        if "b_jump_max_abs" in hdf.columns:
            hdf = (
                hdf[["well_id", "b_jump_max_abs"]]
                .drop_duplicates(subset=["well_id"])
            )
            hdf["well_id"] = hdf["well_id"].astype(str)
            for wid, v in zip(hdf["well_id"], hdf["b_jump_max_abs"]):
                out.setdefault(wid, {})["wpB_b_jump_max_abs"] = float(v) if pd.notna(v) else float("nan")

    # D9
    f_path = root / "formation-uniqueness.parquet"
    if f_path.exists():
        fdf = pd.read_parquet(f_path)
        keep = ["well_id"] + [c for c in WINPATH_B_FORMATION_COLS if c in fdf.columns]
        fdf = fdf[keep].drop_duplicates(subset=["well_id"])
        fdf["well_id"] = fdf["well_id"].astype(str)
        for _, row in fdf.iterrows():
            wid = row["well_id"]
            for c in WINPATH_B_FORMATION_COLS:
                if c in fdf.columns:
                    out.setdefault(wid, {})[f"wpB_{c}"] = (
...
def winpath_b_attach(ev: pd.DataFrame, wid: str) -> pd.DataFrame:
    """Add the 8 winning-path-B columns (D1+D4+D9) to ev for one well.

    All 8 columns are per-well constants so they're broadcast across rows.
    Missing entries become NaN / -1 (= LGB handles natively).
    """
    if not WINPATH_B_ENABLE:
        return ev
    feats = _WINPATH_B_LOOKUP.get(str(wid), {})
    ev = ev.copy()
    for col in WINPATH_B_OUT_COLS:
        if col == "wpB_cluster_id":
            ev[col] = int(feats.get(col, -1))
        else:
            ev[col] = float(feats.get(col, float("nan")))
    return ev
...
def build_well_features(
    ...
    # ── Winning Path B (= D1 + D4 + D9) inject (= exp011 新規) ──────────────
    if WINPATH_B_ENABLE:
        global _WINPATH_B_LOOKUP
        if not _WINPATH_B_LOOKUP:
            try:
                _WINPATH_B_LOOKUP = winpath_b_load()
                print(f"  [winpath B] lookup loaded: {len(_WINPATH_B_LOOKUP)} wells, "
                      f"sample cols={list(_WINPATH_B_LOOKUP.values())[0].keys() if _WINPATH_B_LOOKUP else []}")
            except Exception as _wpb_e:
                print(f"  [winpath B] lookup load failed: {type(_wpb_e).__name__}: {_wpb_e}")
                _WINPATH_B_LOOKUP = {"__failed__": True}
        if "__failed__" not in _WINPATH_B_LOOKUP:
            out = winpath_b_attach(out, wid)
