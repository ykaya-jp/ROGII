...
MODE = "infer"  # exp020 inference: load pretrained weights + predict on hidden test (rule § 6 pretrained models OK)  # orig: train mode runs full Climber + 3-axis grid
...
if MODE in ("train", "cv", "features_only", "ensemble_only"):
...
elif MODE in ("infer", "infer_tabicl"):
    # exp020 inference: dataset mount under /kaggle/input/datasets/<user>/<slug>/artefacts/
    _CANDIDATES = [
        Path("/kaggle/input/datasets/ky7240/rogii-exp020-pretrained-weights/artefacts"),
        Path("/kaggle/input/rogii-exp020-pretrained-weights/artefacts"),
    ]
    ARTEFACT_DIR = None
    ...
    if ARTEFACT_DIR is None:
        # glob fallback
        for _p in Path("/kaggle/input").glob("**/artefacts"):
            if _p.is_dir():
                ARTEFACT_DIR = _p
                break
    assert ARTEFACT_DIR is not None, f"artefacts dir not found in /kaggle/input"
