...
# # ROGII exp007 — Edge Q + Edge M + 自前 LGB×3+CB (LB target: 9.5-9.7 帯)
#
# Self-contained Kaggle GPU script that extends exp006 with:
...
#      公開議論未出 (subagent J `kaggle-deepdive.dense.md` §5.1 候補 A)。
#      期待: CV-LB gap 縮小 → +0.05〜+0.20 ft の LB 改善。
#   2. **Edge M (visible-as-typewell self-alignment)** — host pptx Slide 9
#      公式推奨 (公開 kernel で実装ゼロ): visible 部 horizontal GR を local
#      reference に hidden 部と self-NCC、4 features (selfaln_d / selfaln_score
#      / selfaln_vs_typewell_d / selfaln_conflict_score) を build_well_features に
#      追加。期待: -0.30〜-0.60 ft。
#   3. **自前 LGB×3 (lr=0.02/0.05/0.10) + CB (lr=0.05) を Edge Q fold で
#      5-fold OOF 学習** — karnakbaev pretrained 5 base に diversity-lr で
#      4 base 加算、karnakbaev OOF も Edge Q fold で再計算して weight
#      constellation の consistency を保つ。
#   4. **9-base Ridge meta** (= 5 karnakbaev + 4 own、positive=True、no
#      intercept) を Edge Q fold OOF で fit、test predictions に適用。
#   5. **TabICL は dormant** (= exp006 で投入が逆に LB 悪化したため、本 exp
#      では同じ過ちを繰り返さない)。
#
# Pipeline (MODE = 'infer_edge_qm'):
...
#   d. Compute Edge M FE for both train and test (= visible-as-typewell self-NCC).
...
#
# This file is for the team's exp007 entry only.
...
#       | "infer_tabicl" | "infer_edge_qm"
#   infer_edge_qm: exp007 path = Edge Q (typewell hash GroupKFold) + Edge M
#                  (visible-as-typewell self-NCC FE) + 自前 LGB×3 + CB train +
...
print("Edge Q + Edge M helpers loaded ✓")
...
def build_well_features(
    ...
    out = pd.DataFrame({
        ...
        **tw_diff_beam,
        **tw_diff_sc,
        # Edge M (visible-as-typewell self-alignment, exp007 新規 4 cols)
        **edge_m_out,
    })
...
if MODE == "features_only":
...
elif MODE == "infer_edge_qm":
    # ── exp007 path: Edge Q + Edge M + 自前 4 base + 9-base Ridge meta ───────
    feature_cols   = am.load_json(am.FEATURES_LIST)
    pp_params      = am.load_json("postproc_params")
    best_alpha     = float(pp_params["alpha"])
    best_tau       = pp_params.get("tau")

    section(log, "exp007: build test features (live test wells, with Edge M)")
    with timer(log, "test feature engineering"):
        test_df = build_dataset(TEST_DIR, is_train=False,
                                max_wells=DEBUG_MAX_WELLS, n_jobs=NCPU)
    log.info(f"Test features: {test_df.shape}")
    ...
    section(log, "exp007: load karnakbaev pretrained 5 base + predict test")
    import lightgbm as _lgb2
    from xgboost import XGBRegressor as _XGB2
    from catboost import CatBoostRegressor as _CB2
    base_models = {}
    ...
    m_cb = _CB2(); m_cb.load_model(str(ARTEFACT_DIR / "final_cb.cbm"))
    base_models["cb"] = m_cb
    test_preds_kb = predict_all(base_models, test_df[feature_cols])
    ...
    try:
        ...
        section(log, "exp007: karnakbaev OOF refit under Edge Q fold")
        ...
        with timer(log, "kb OOF refit (5 fold predict)"):
            for k in ACTIVE_MODELS:
                preds_full = base_models[k].predict(
                    train_df_kb[feature_cols].values
                ) if k != "xgb" else base_models[k].predict(
                    train_df_kb[feature_cols]
                )
        ...
        if OWN_BASE_ENABLE and not DEBUG_SKIP_OWN_TRAIN:
            section(log, "exp007: 自前 4 base train (Edge Q fold)")
            own_t0 = time.perf_counter()
            X_kb_full = train_df_kb[feature_cols]
        ...
        log.error(f"exp007 train_df phase failed: {type(e).__name__}: {e}")
    ...
    if own_ok and y_kb is not None:
        section(log, "exp007: 9-base Ridge meta fit + apply")
        try:
    ...
    if used_path == "fallback":
        section(log, "exp007 fallback: karnakbaev 5-base NM blend")
        nm_w = am.load_json(am.ENSEMBLE_WEIGHTS)
    ...
    # ── Step G: post-processing ────────────────────────────────────────────
    section(log, f"exp007: post-processing + SG smooth (path={used_path})")
    test_delta_pp = apply_postproc(test_df, test_delta, best_alpha, best_tau)
    test_delta_sg = sg_smooth_per_well(test_df, test_delta_pp)
    if y_kb is not None:
        clip_lo = float(y_kb.min()) * 1.5
    ...
    log.error(f"Unknown MODE='{MODE}'. "
              ...
              f"cv | features_only | ensemble_only")
