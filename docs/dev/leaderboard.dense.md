...
| 2026-05-11 00:32 | 52528863 | exp005 v3 | karnakbaev blend + Edge S + **Edge R** (test-time online learning, continued training on test visible TVT_input) | (no 自前 CV) | **10.387** ⚠️ | - | exp005 v2 → +0.184 悪化 | - | kernel: `ky7240/rogii-exp005-cache-blend` v3。**🚨 Edge R 単独効果 = +0.184 ft 悪化** (公開 698002 -0.37 と逆方向、差 0.554)。kernel log で実装 正常動作確認、原因仮説: base model 依存 + w_R=0.5 過大 + augmentation 不足。**学び: pane 2 「最高 ROI 1 手」 revoke、全 文献 hack を AB test で必ず verify** |
| 2026-05-11 03:15 | 52532035 | **exp008 v2** | exp007 base + **案 D (Kalman/PF on dTVT, AR(1) Yule-Walker MLE + global shrinkage, 7 features)** + Edge S | (no 自前 CV) | **9.957** ✅ | - | exp005 v2 → **-0.246 改善** | - | kernel: `ky7240/rogii-exp008-case-d-kalman` v1。**案 D Kalman 単独効果 = -0.246 ft 改善 verified** (= 想定 -0.3〜-0.6 範囲内、下限寄り)。**Gold cutoff (9.865) まで -0.092 ft、Top 20 ライン ぎりぎり外**。AR(1) phi=0.999 の MAP estimate + global prior shrinkage が effective、hidden zone 末端 systematic bias 抑制成功。**学び**: (a) **加算的 inject (= 自前 4 base なし)** は確実に effective、(b) base improve は karnakbaev pretrained に Kalman feature 加算で安定、(c) 9 切りには **案 E + Edge O 累積 (= exp009 v2) で更 -0.5 ft 必要** |
